...
export const store = new Vuex.Store({  //actions이 mutations을 실행해주고, mutations가 state에 값을 주는 것.
  ...
  mutations: {
    SET_LIST(state, list) {   // state에 저장  // mutation에도 state가 자동으로 생성. 단, state라고 이름 정해진것은 아님.
      state.list = list;
    }
  },
  actions: {
    ...
    FETCH_LIST(context, pageName) {  //두개의 인자를 받아야함. context는 정해져 있는 것(자동으로 생겨서 받아오게 되는 것). // context에는 commit이 들어있음. 
